...
Perceptron::Perceptron(size_t inputs, Activation::Type type, float bias)
    : bias(bias), activation(type)
{
...
float Perceptron::run(std::vector<float> x)
{
    ...
    return activation.apply(inner_product(x.begin(), x.end(), weights.begin(), (float)0.0));
}
...
MultilayerPerceptron::MultilayerPerceptron(std::vector<size_t> layers, Activation::Type type, float bias, float eta, float dropout)
...
void MultilayerPerceptron::printStructure(std::ostream& out) const
{
    ...
    for (size_t i = 0; i < layers.size(); ++i)
    {
        ...
        out << std::left
            ...
            << std::setw(8) << layers[i]
            << std::setw(12) << (i == 0 ? "-" : Activation(actType).toString())
                << std::setw(12) << params
                << "\n";
    }
...
float MultilayerPerceptron::backPropagation(std::vector<float> x, std::vector<float> y)
{
    ...
    {
        d.back()[i] = network.back()[i].activation.derivative(values.back()[i]) * error[i];
    }
    ...
    for (size_t i = network.size() - 2; i > 0; i--)
    {
        for (size_t j = 0; j < network[i].size(); j++)
        {
            ...
            d[i][j] = network[i][j].activation.derivative(values[i][j]) * fwdErr;
        }
    }
